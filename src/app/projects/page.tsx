	...
	return (
		<section className="max-w-[90rem] my-14 mx-auto">
			...
			<p className="md:w-[35rem] -mt-4 mb-5 dark:text-stone-400">
				I&apos;ve worked on a number of projects over the years but these are the ones I&apos;m most proud of. Some are
				personal projects, some are collaborations and some are client projects.
			</p>
				...
				{projects.map((project, i) => (
					...
					>
						...
						<Slide className="flex-1" towards={i % 2 === 0 ? "right" : "left"} delay={i * 0.1}>
							...
							{project?.development && (
								<small className="block mb-3 text-red-500">This project is currently under development.</small>
							)}
							{project?.credentials && (
								<div className="mb-2 flex flex-col text-green-700">
									<small>
										Email: <span className="select-all">{project.credentials?.email}</span>
									</small>
									<small>
										Password: <span className="select-all">{project.credentials?.password}</span>
									</small>
								</div>
							)}
